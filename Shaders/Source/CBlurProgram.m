...
@property (readwrite, nonatomic, assign) GLint blurRadiusUniform;
@property (readwrite, nonatomic, assign) BOOL blurRadiusChanged;
...
@synthesize blurRadiusUniform;
@synthesize blurRadiusChanged;
...
- (void)update
    ...
    if (blurRadiusChanged == YES)
        {
        if (blurRadiusUniform == -1)
            {
            blurRadiusUniform = glGetUniformLocation(self.name, "u_blurRadius");
            }

        glUniform1f(blurRadiusUniform, blurRadius);;
        blurRadiusChanged = NO;
        AssertOpenGLNoError_();
        }
...
- (void)setBlurRadius:(GLfloat)inBlurRadius
    {
